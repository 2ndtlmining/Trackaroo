...
- **Full validation before finishing a task**: backend `python -m pytest -q` (408 tests, run from the repo root), then from `web/`: `npm run check`, `npm test`, then `npm run test:e2e`.
