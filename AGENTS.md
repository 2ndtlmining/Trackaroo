...
- **Unit tests**: `npm test` (Vitest, 204 tests, ~2s)
...
- **Full validation before finishing a task**: backend `python -m pytest -q` (461 tests, run from the repo root), then from `web/`: `npm run check`, `npm test`, then `npm run test:e2e`.
